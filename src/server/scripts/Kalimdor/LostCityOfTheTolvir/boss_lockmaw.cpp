...
public:
    ...
    struct boss_lockmawAI : public ScriptedAI
    {
        ...
        {
            Summons.DespawnAll();
            if (IsHeroic())	//Summon Augh!
			{
				me->SummonCreature(49045, SummonLocations[4], TEMPSUMMON_CORPSE_DESPAWN);
				//disable Loot - only free it, when augh ist dead
...
public:
    ...
    struct mob_aughAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if(pLockmaw)
				pLockmaw->SetFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE);
        }
