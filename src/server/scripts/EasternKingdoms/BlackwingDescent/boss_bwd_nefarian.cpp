...
/* ScriptData
 SDName: boss_bd_nefarian
 SD%Complete: 60%
 SDComment: Not blizzlike at all
...
//FIXME: Overload, Enrage Timer, distance between onyxia and nefarian, onyxia spawn pos, bone_warriors should
//		 be revived if nefarian spits on them
...
const Position MiddleRoomLocation = {-103.057961f, -222.698685f, 11.0f, 0.0f};

const Position Phase2Location = {-103.057961f, -222.698685f, 46.959911f, 0.0f};

const Position OnyxiaLocation = {-143.726f, -191.979f, 18.3750f, 5.7589f};

Creature* pOnyxia = NULL;
Creature* pPrototype = NULL;
Creature* pNefarian = NULL;
...
public:
    ...
    struct boss_bd_nefarianAI : public BossAI
    {
        ...
        void Reset()
        {
			...
			if(!pOnyxia)
			{
				me->SummonCreature(41270, OnyxiaLocation, TEMPSUMMON_CORPSE_DESPAWN);
			}
        }
        ...
        void UpdateAI(const uint32 diff)
        {
			...
			//Phase switch "Onyxia died" handled via Action!
            if (((me->GetHealth()*100 / me->GetMaxHealth()) < 70) && (Phase == 1))
            {
                Phase = 2;
            }

            if ( (SpawnCount == 0 && (Phase == 2)) || ( (me->GetHealth()*100 / me->GetMaxHealth() < 60) && Phase == 2 ) )	//Nefarian should switch to phase 3 if the prototypes are dead as of video guide
            {
                Phase = 3;
            }
            ...
            if (Phase == 1)
            {
				...
				//We need to specify the distance!
				/*if(pOnyxia != NULL && pNefarian != NULL)
					if(me->GetDistance2d(pOnyxia->GetPositionX(), pOnyxia->GetPositionY()) < 5)
					{
						DoCast(me, SPELL_CHILDREN_OF_DEATHWING);
						pOnyxia->CastSpell(pOnyxia, SPELL_CHILDREN_OF_DEATHWING, false);
					}*/
            ...
            if (Phase == 3)
            {
				...
				if(!phase3_entered)
				{
					...
					me->SetFlying(false);
					me->GetMotionMaster()->MovePoint(2, MiddleRoomLocation);
...
public:
    ...
    struct npc_animated_bone_warriorAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
			...
			//After 50s the energy of the npcs is empty
